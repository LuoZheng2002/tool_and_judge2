...
class GPT5Interface(JudgeModelInterface, ToolModelInterface):
    ...
    async def generate_tool_call_async(
        ...
        preconstructed_tools: Optional[List[Dict[str, Any]]] = None,
    ) -> str:
        """
        ...
        """
        # Use preconstructed tools if provided, otherwise build them
        if preconstructed_tools is not None:
            tools = preconstructed_tools
            print("[TEST MODE] Using preconstructed tools from JSON file")
        else:
