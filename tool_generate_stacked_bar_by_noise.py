...
def generate_stacked_bar_chart_by_noise(model_names: list, output_dir: str, result_dir: str,
                                         language: str,
                                         translate_mode: str,
    ...
    for model_name in model_names:
        ...
        try:
            lang_data = load_multi_model_statistics([model_name], result_dir, language, translate_mode)
            for nm in noise_modes:
                data_dict[model_name][nm] = lang_data[model_name][nm]
        except (ValueError, KeyError) as e:
            print(f"Warning: Could not load data for {model_name}, {language}: {e}")
    ...
    if translate_mode == "NT":
        title = f"Tool Calling Errors - English"
    else:
        title = f"Tool Calling Errors - {language} - {translate_mode}"
    output_name = f"stacked_bar_by_noise_{language}_{translate_mode}.pdf"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate stacked bar charts comparing models showing error type distributions by noise mode."
    )
    parser.add_argument(
        "models",
        nargs="+",
    ...
    parser.add_argument(
        "language",
        help="Language name for filtering (e.g., Chinese, Hindi, Igbo)"
    )
    parser.add_argument(
        "translate_mode",
    ...
    parser.add_argument(
        "--output-dir",
        default="tool/plots/stacked_bars_by_noise",
        help="Directory to save chart images (default: tool/plots/stacked_bars_by_noise)"
    )
    parser.add_argument(
        "--result-dir",
        default="tool/result",
    ...
    print(f"\n{'='*60}")
    print(f"Generating stacked bar charts comparing models by noise mode")
    ...
    generate_stacked_bar_chart_by_noise(
        args.models,
        args.output_dir,
        args.result_dir,
        args.language,
