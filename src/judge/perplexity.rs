...
use crate::{
    ...
    judge::{
        ...
        generate_dataset::{
            PerplexityDatasetMaskEntry, TwoAnswersSameLangEntry, generate_perplexity_dataset_mask,
            generate_two_answers_same_lang_dataset, get_valid_perplexity_indices,
        },
    },
...
#[pyfunction]
pub fn perplexity_prepare_response_input(config: &JudgeConfig, debug_limit: Option<usize>) {
    ...
    let languages = match &config.experiments {
        JudgeExperiments::Vllm {
            ...
            .iter()
            .map(|exp| exp.lang.clone())
            .collect::<Vec<String>>(),
        JudgeExperiments::HuggingFace { .. } => {
            panic!("HuggingFace experiments do not support perplexity_prepare_response_input")
        }
    };
    ...
    for lang in languages.iter() {
        ...
        let dataset_entries = load_json_lines(&dataset_path)
        ...
        let perplexity_mask_entries_parsed: IndexMap<usize, PerplexityDatasetMaskEntry> =
            perplexity_mask_entries
                .into_iter()
                .map(|entry| {
                    let parsed: PerplexityDatasetMaskEntry = serde_json::from_value(entry)
    ...
    println!(
        "Response input dataset for languages {:?} written to {}",
        languages, output_file_path
    );
}
...
pub fn perplexity_prepare_generate_styled_answers_input(
    ...
    let languages = match &config.experiments {
        JudgeExperiments::Vllm {
            ...
            .collect::<Vec<String>>(),
        JudgeExperiments::HuggingFace { .. } => {
            panic!("HuggingFace experiments do not support perplexity_prepare_styled_answers_input")
        }
    };
    ...
    for lang in languages.iter() {
        ...
        let processed_ids: HashSet<usize> = match load_json_lines(&result_path) {
            Ok(entries) => entries
                .into_iter()
                .map(|entry| {
                    let parsed: StyledAnswersEntry = serde_json::from_value(entry)
    ...
    println!(
        "Styled answers input dataset for languages {:?} written to {}",
        languages, output_file_path
    );
}
...
pub fn perplexity_prepare_generate_perplexity_aggregated_input(
    ...
    let languages = match &config.experiments {
        JudgeExperiments::Vllm { .. } => {
            panic!("Vllm experiments do not support perplexity_prepare_aggregated_input")
        }
        JudgeExperiments::HuggingFace {
            perplexity_experiments,
        } => perplexity_experiments
    ...
    let mut aggregated_entries: Vec<GeneratePerplexityAggregatedInputEntry> = Vec::new();
    for lang in languages.iter() {
        ...
        // parse all entries
        let styled_answers_entries_parsed: IndexMap<usize, StyledAnswersEntry> =
            styled_answers_entries
                .into_iter()
                .map(|entry| {
                    let parsed: StyledAnswersEntry = serde_json::from_value(entry)
                        .expect("Failed to parse correct one answer entry");
                    (parsed.index, parsed)
                })
                .collect();
    ...
    let combined_entries_serialized: Vec<serde_json::Value> = aggregated_entries
        .into_iter()
    ...
    println!(
        "Concatenated perplexity dataset for languages {:?} written to {}",
        languages, output_file_path
    );
}
...
pub fn perplexity_dispatch_response_results(config: &JudgeConfig) {
    ...
    let languages = match &config.experiments {
        JudgeExperiments::Vllm {
            ...
            .collect::<Vec<String>>(),
        JudgeExperiments::HuggingFace { .. } => {
            panic!("HuggingFace experiments do not support perplexity_dispatch_response_results")
        }
    };
    let input_file_path = perplexity_generate_response_output_file_path(config);
    println!(
        "Dispatching response results for model: {}, languages: {:?}, input file: {}",
        model_safe_name, languages, input_file_path
    );
    for lang in languages.iter() {
        let result_path = JUDGE_BASE_RESULT_PATH
            .join(&model_safe_name)
...
pub fn perplexity_dispatch_styled_answers_results(config: &JudgeConfig) {
    ...
    let languages = match &config.experiments {
        ...
        JudgeExperiments::HuggingFace { .. } => {
            panic!(
                "HuggingFace experiments do not support perplexity_dispatch_styled_answers_results"
            )
        }
    };
    let input_file_path = perplexity_generate_styled_answers_output_file_path(config);
    println!(
        "Dispatching styled answers results for model: {}, languages: {:?}, input file: {}",
        model_safe_name, languages, input_file_path
    );
    for lang in languages.iter() {
        let result_path = JUDGE_BASE_RESULT_PATH
...
pub fn perplexity_dispatch_generate_perplexity_results(config: &JudgeConfig) {
    ...
    let languages = match &config.experiments {
        JudgeExperiments::HuggingFace {
            ...
            .collect::<Vec<String>>(),
        JudgeExperiments::Vllm { .. } => {
            panic!("Vllm experiments do not support perplexity_dispatch_perplexity_results")
        }
    };
    let input_file_path = perplexity_generate_perplexity_aggregated_output_file_path(config);
    println!(
        "Dispatching perplexity results for model: {}, languages: {:?}, input file: {}",
        model_safe_name, languages, input_file_path
    );
    for lang in languages.iter() {
        let correct_result_path = JUDGE_BASE_RESULT_PATH
        ...
        let incorrect_serialized = incorrect_result_entries
            .iter()
            .map(|e| {
                serde_json::to_value(e).expect("Failed to serialize incorrect perplexity entry")
            })
            .collect::<Vec<_>>();
