...
fn bfcl_param_to_deepseek_param(bfcl_parameter: &BfclParameter, required: bool) -> DeepSeekParameter {
    let BfclParameter {
        ty: bfcl_type,
...
impl ModelInterface for DeepSeekInterface {
    ...
    async fn translate_tool_question_async(
        ...
        let response_str = fut.await.expect("DeepSeek tool question translation failed");
        let response_str = Python::attach(|py| {
            response_str
                .extract::<String>(py)
                .expect("Failed to extract response string")
    ...
    fn postprocess_tool_calls(
        ...
        let response: PostprocessResponse = serde_json::from_str(&json_result)
        ...
        if response.success {
            let calls_list = response.calls.ok_or_else(|| EvaluationError::ParsingError {
            ...
            for call_map in calls_list {
                ...
                if call_map.len() != 1 {
                    return Err(EvaluationError::ParsingError {
                        error_message: format!("Expected single-entry function call map, got {} entries", call_map.len()),
                        raw_output: raw_output.to_string(),
                    });
                }
                ...
                let original_function_name = {
                    ...
                    name_mapper_borrow.get_original_name(func_name)
                };
            ...
            let error_type = response.error_type.unwrap_or_else(|| "UNKNOWN_ERROR".to_string());
