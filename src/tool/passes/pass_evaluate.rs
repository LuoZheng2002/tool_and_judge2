...
use crate::tool::bfcl_formats::BfclParameter;
...
#[pyfunction]
pub fn pass_evaluate(config: &ToolConfig) {
    ...
    for experiment in config.experiments.iter() {
        ...
        let ground_truth_entries =
            load_json_lines(&ground_truth_file_path).expect("Failed to load ground truth file");
        let ground_truth_entries_parsed: HashMap<String, BfclGroundTruthEntry> =
            ground_truth_entries
                .into_iter()
                .map(|entry| {
                    let parsed_entry: BfclGroundTruthEntry =
                        serde_json::from_value(entry).expect("Ground truth entry has wrong format");
                    (parsed_entry.id.clone(), parsed_entry)
                })
                .collect();
        let post_translate_entries: Option<HashMap<String, ParseOutputEntry>> = {
            ...
            match post_translate_mode {
                ...
                PostTranslateMode::PostTranslate => {
                    let post_translate_file_name =
                        PostTranslateFileName::from_config_experiment(experiment);
                    let post_translate_file_name_str = format!(
                        "{}.jsonl",
                    ...
                    let post_translate_entries = load_json_lines(&post_translate_file_path)
                        .expect("Failed to load post translate output file");
                    let post_translate_entries_parsed: HashMap<String, ParseOutputEntry> =
                        post_translate_entries
                            .into_iter()
                            .map(|entry| {
                                let parsed_entry: ParseOutputEntry = serde_json::from_value(entry)
                                    .expect("Post translate output entry has wrong format");
                                (parsed_entry.id.clone(), parsed_entry)
                            })
                            .collect();
                    Some(post_translate_entries_parsed)
                }
            }
        };
        let mut evaluate_entries: Vec<EvaluateEntry> = vec![];
        let ids = parse_output_entries_parsed
            .keys()
            .cloned()
            .collect::<Vec<String>>();
        for id in ids.iter() {
            let result_entry = match &post_translate_entries {
                Some(post_translate_entries) => post_translate_entries
                    .get(id)
                    .expect("Post translate entry should exist"),
                None => parse_output_entries_parsed
                    .get(id)
                    .expect("Parse output entry should exist"),
            };
            let dataset_entry = dataset_entries_parsed
                .get(id)
                .expect("Dataset entry should exist");
            let ground_truth_entry = ground_truth_entries_parsed
                .get(id)
                .expect("Ground truth entry should exist");
            let evaluate_entry = evaluate_entry(result_entry, dataset_entry, ground_truth_entry);
            evaluate_entries.push(evaluate_entry);
        }
        evaluate_entries.sort_by(|a, b| compare_id(&a.id, &b.id));
        let evaluate_entries_serialized: Vec<serde_json::Value> = evaluate_entries
            .into_iter()
            .map(|entry| serde_json::to_value(entry).unwrap())
            .collect();
        write_json_lines_to_file(&evaluate_file_path, &evaluate_entries_serialized)
            .expect("Failed to write evaluate entries to file");
        println!("Wrote evaluate entries to file: {:?}", evaluate_file_path);
    }
    println!("Pass evaluate completed for model {}", model_safe_name);
}

fn evaluate_entry(
    result_entry: &ParseOutputEntry,
    test_case_entry: &BfclDatasetEntry,
    ground_truth_entry: &BfclGroundTruthEntry,
) -> EvaluateEntry {
    let id = result_entry.id.clone();
    assert_eq!(
        id, test_case_entry.id,
        "result entry id should match test case entry id"
    );
    assert_eq!(
        id, ground_truth_entry.id,
        "result entry id should match ground truth entry id"
    );
    let functions = match &result_entry.result {
        Ok(funcs) => funcs,
        Err(e) => {
...
fn check_recursively_for_required_and_unexpected(
    ...
    for (parameter_name, parameter_value) in parameters.iter() {
        ...
        let sub_parameters_map: IndexMap<String, serde_json::Value> = sub_parameters_map
            .iter()
