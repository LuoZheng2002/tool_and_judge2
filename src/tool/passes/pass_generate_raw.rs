...
use crate::{
    config::{Model, ToolConfig, ToolExperiment, TranslateMode}, models::{function_name_mapper::{self, FunctionNameMapper}, model_interface::get_model_interface}, tool::{
        base_path::{BASE_DATASET_PATH, BASE_RESULT_PATH},
        bfcl_formats::{BfclDatasetEntry, BfclFunctionDef},
        experiments::{DatasetFileName, GenerateRawFileName, PreTranslateMode, PromptTranslateMode}, passes::pass_pre_translate::PreTranslateQuestionEntry,
...
pub fn pass_generate_raw_prepare_aggregated_input(config: &ToolConfig) {
    ...
    for experiment in config.experiments.iter() {
        ...
        let translated_question_entries: Option<HashMap<String, PreTranslateQuestionEntry>> = match result_file_name.3 {
            PreTranslateMode::PreTranslate => {
                let pre_translate_result_file_name_str = format!("{}.jsonl", serde_json::to_string(&dataset_file_name).unwrap());
                let pre_translate_result_file_path = BASE_RESULT_PATH
                    .join(&model_safe_name)
                    .join("pre_translate")
                    .join(&pre_translate_result_file_name_str);
                let pre_translate_entries = load_json_lines(&pre_translate_result_file_path).expect("Failed to open pre-translate result file");
                let pre_translate_entries_parsed: HashMap<String, PreTranslateQuestionEntry> = pre_translate_entries
                    .into_iter()
                    .map(|entry| {
                        let parsed_entry: PreTranslateQuestionEntry =
                            serde_json::from_value(entry).expect("Pre-translate entry has wrong format");
        ...
        println!("For result file {}, total entries: {}, existing entries: {}, missing entries to generate: {}",
            result_file_path.to_str().unwrap(),
            ids.len(),
        ...
        for id in missing_ids.iter() {
            let dataset_entry = dataset_entries_parsed.get(id).expect("Missing ID should exist in dataset entries");
            let question = match &translated_question_entries {
                Some(translated_map) => {
                    let translated_entry = translated_map.get(id).expect(&format!("Missing translated entry for ID {}", id));
                    translated_entry.translated_question.clone()
                },
                None => {
                    dataset_entry.question[0][0].content.clone()
                },
            };
            let tools = model_interface.generate_tool_definitions(&dataset_entry.function, &function_name_mapper);
    ...
    println!("Wrote aggregated input file to {}", aggregated_input_file_path);
}
...
pub fn pass_generate_raw_dispatch_results(config: &ToolConfig) {
    ...
    let output_entries = load_json_lines(&output_file_path).expect("Failed to open aggregated output file");
    let output_entries_parsed: HashMap<(String, GenerateRawFileName), GenerateRawAggregatedOutputEntry> = output_entries
        .into_iter()
        .map(|entry| {
            ...
            ((parsed_entry.id.clone(), parsed_entry.file_name.clone()), parsed_entry)
        })
        .collect();
    for experiment in config.experiments.iter() {
        let dataset_file_name = DatasetFileName::from_config_experiment(experiment);
        ...
        let ids: HashSet<String> = dataset_entries
            ...
            .collect();
        let mut result_existing_entries: Vec<GenerateRawEntry> = match load_json_lines(&result_file_path) {
            Ok(existing_results) => existing_results
                .into_iter()
                .map(|entry| {
                        serde_json::from_value(entry).expect("Result entry has wrong format")
        ...
        let result_existing_ids: HashSet<String> = result_existing_entries
            ...
            .collect();
        let result_missing_ids: HashSet<String> = ids
            .difference(&result_existing_ids)
            .cloned()
            .collect();
        ...
        result_existing_entries.sort_by(|a, b| a.id.cmp(&b.id));
...
pub fn generate_function_name_mapper_file(){
    let output_file_path = BASE_DATASET_PATH
        .join("function_name_mapper.jsonl");
    ...
    let function_name_mapper_entries: Vec<FunctionNameMapperEntry> = original_to_sanitized
        .into_iter()
        .map(|(original_function_name, sanitized_function_name)| FunctionNameMapperEntry {
    ...
    println!("Wrote function name mapper file to {}", output_file_path.to_str().unwrap());
}

pub fn get_function_name_mapper() -> FunctionNameMapper {
    let function_name_mapper_file_path = BASE_DATASET_PATH
        .join("function_name_mapper.jsonl");
    ...
    let function_name_mapper_entries_parsed: Vec<FunctionNameMapperEntry> = function_name_mapper_entries
        .into_iter()
        .map(|entry| {
