...
def generate_stacked_bar_chart_mitigation(model_name: str, output_dir: str, result_dir: str,
                                          language: str,
                                          max_height: float = None,
                                          output_format: str = "png") -> None:
    """
    Generate a stacked bar chart for a given model showing error type distributions.
    Shows only 5 bars: FT, PT, PRE, POST, and NT, all in NO_NOISE mode.
    ...
    # Prepare data for plotting - FT, PT, PRE, POST, NT in NO_NOISE mode
    translate_modes = ["FT", "PT", "PRE", "POST", "NT"]
    ...
    title = f"{model_name}'s performance on {language} datasets with different\nmitigation strategies (no semantic perturbation added)"
    output_name = f"stacked_bar_mitigation_{model_name}_{language}.{output_format}"
    ...
    # Add "Mitigation Strategy" label below the bar labels
    ax.set_xlabel('Mitigation Strategy', fontsize=15, fontweight='bold')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir",
        default="tool/plots/stacked_bars_mitigation",
        help="Directory to save chart images (default: tool/plots/stacked_bars_mitigation)"
    )
    parser.add_argument(
        "--result-dir",
        default="tool/result",
    ...
    for model in args.models:
        print(f"\n{'='*60}")
        print(f"Generating stacked bar charts (mitigation strategies) for {model}")
        print(f"{'='*60}")

        for language in languages:
            print(f"\nProcessing language: {language}")
            generate_stacked_bar_chart_mitigation(
                model,
                args.output_dir,
                args.result_dir,
                language,
